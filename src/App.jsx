...
function parseJSON(text,type="object"){
  ...
  const open=type==="array"?"[":"{";const close=type==="array"?"]":"}";
  const s=clean.indexOf(open);const e=clean.lastIndexOf(close);
...
function ApiKeyScreen({onDone}){
  ...
  return <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'DM Sans',sans-serif",color:C.text,display:"flex",flexDirection:"column"}}>
    <style>{CSS}</style>
    <nav style={{padding:"16px 28px",borderBottom:`1px solid ${C.border}`}}><Logo/></nav>
      ...
      <div className="fu" style={{maxWidth:460,width:"100%",background:C.card,borderRadius:22,border:`1px solid ${C.border}`,overflow:"hidden"}}>
        ...
        <div style={{padding:32}}>
          ...
          <button onClick={test} disabled={testing||!key.trim()} className="pbtn"
            style={{width:"100%",marginTop:14,padding:"15px",background:testing||!key.trim()?C.border:`linear-gradient(135deg,${C.blue},${C.purple})`,color:testing||!key.trim()?C.muted:"#fff",border:"none",borderRadius:12,fontSize:15,fontWeight:700,fontFamily:"'Syne',sans-serif",cursor:testing||!key.trim()?"not-allowed":"pointer",boxShadow:!testing&&key.trim()?`0 8px 28px ${C.blue}40`:"none"}}>
      ...
      <div className="fu1" style={{background:C.card,borderRadius:22,border:`1px solid ${C.border}`,overflow:"hidden",marginBottom:20}}>
        ...
        <div style={{padding:32}}>
          {!useManual?(<>
            ...
            <textarea value={manualInput} onChange={e=>setManualInput(e.target.value)} placeholder="Misal: 4 saal SQA experience. Manual testing, Selenium automation, Postman API testing, JIRA. Agile/Scrum teams mein kaam kiya. Thora project management bhi. Remote ya relocation chahiye..." style={{width:"100%",minHeight:155,background:C.bg,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px",color:C.text,fontSize:14,fontFamily:"'DM Sans',sans-serif",resize:"vertical",lineHeight:1.7,outline:"none"}}/>
            <p style={{color:C.muted,fontSize:12,marginTop:8}}>Jitna detail, utna behtar result</p></>
          )}
...
function ATSScreen({ats,loading,onHunt,onBack}){
  ...
  return <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'DM Sans',sans-serif",color:C.text}}>
    ...
    <header style={{padding:"14px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,background:C.bg+"EE",backdropFilter:"blur(12px)"}}>
      <Logo/>
      <div style={{display:"flex",gap:10,alignItems:"center"}}>
        {ats&&!loading&&<Tag color={sc(ats.atsScore)}>✓ ATS {ats.atsScore}%</Tag>}
...
export default function App(){
  ...
  const optimize=async()=>{
    ...
    try{
      ...
      console.error(e);
      setAts({candidateName:"Your Profile",currentRole:"SQA Engineer",experience:"Professional",atsScore:72,optimizedResume:"Error: "+e.message+"\n\nPlease go back and try again.",skills:["SQA","Manual Testing","Automation","JIRA","Agile"],jobTitles:["QA Engineer","SQA Engineer"],keywords:["quality assurance","software testing","agile"],tools:["JIRA","Selenium","Postman"],suggestions:["Go back and try again"]});
    }
    setAtsLoading(false);
  };
  ...
  if(screen==="apikey") return <ApiKeyScreen onDone={()=>setScreen("home")}/>;
